...
/**
 * @brief Cache the config for quick access.
 * @param config the config to cache
 */
void FALCON_ROUTINE_API cache_config(physics::config::core::ConfigSP config);

/**
 * @brief Reads the current config. Checks the cache first before
 * requesting from the instrument hub.
 * @param timeout_ms the timeout in milliseconds to wait if a request is needed
 * @return The config if successful
 */
physics::config::core::ConfigSP FALCON_ROUTINE_API read_config(int timeout_ms);
...
* @param search_domain the domain to check for safe voltages
* @param timeout_ms the timeout in milliseconds to wait if a request is
needed
